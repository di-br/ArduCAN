...
int MCP2515::Init(uint32_t CAN_Bus_Speed, uint8_t Freq) {
  if(CAN_Bus_Speed>0) {
	...
	for(i=0; i<MAX_SPEEDS; i++) {
	  if(_init(CAN_SPEEDS[i], Freq, 1, true)) {
...
int MCP2515::whatSpeed() {
  // return set Baud rate
  return (int)savedBaud;
}

void MCP2515::Reset() {
  SPI.beginTransaction(canSPISettings);
  //digitalWrite(_CS,LOW);
  RESET(MCP_CS_FAST_PIN);
...
bool MCP2515::Interrupt() {
  ...
  return (!IS_SET(MCP_INT_FAST_PIN));
}
